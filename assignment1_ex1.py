...
def fit_func(inputs, targets, degree):
    ...
    coeffs = np.linalg.inv(X.T @ X) @ X.T @ targets
    return coeffs
    ##your code ends here
...
##your code starts here
# Fit a 3rd degree polynomial to data
coeffs_fitted = fit_func(x_ex1_train, y_ex1_train, 3)
y_pred_train = any_degree_polynomial(x_ex1_train, coeffs_fitted)

# Plot the data and the fitted polynomial
plt.figure(figsize=(8, 5))
plt.scatter(x_ex1_train, y_ex1_train, label='Training Data', color='blue')
plt.plot(x_ex1_train, y_pred_train, label='Fitted Polynomial (Degree 3)', color='red')
plt.legend()
plt.savefig("exercise_1_4_plot.png")
plt.show()
...
def mse(predicted_values, targets):
    ##your code starts here
    return np.mean((predicted_values - targets) ** 2)
    ##your code ends here
...
for d in degrees:
    coeffs = fit_func(x_ex1_train, y_ex1_train, d)
    y_pred_train = any_degree_polynomial(x_ex1_train, coeffs)
    y_pred_val = any_degree_polynomial(x_ex1_val, coeffs)

    train_errors.append(mse(y_pred_train, y_ex1_train))
    val_errors.append(mse(y_pred_val, y_ex1_val))

plt.figure(figsize=(8, 5))
plt.plot(degrees, train_errors, label="Train MSE", marker='o')
plt.plot(degrees, val_errors, label="Validation MSE", marker='o')
plt.xlabel("Polynomial Degree")
plt.ylabel("MSE")
plt.legend()
plt.savefig("exercise_1_7_plot.png")
plt.show()
